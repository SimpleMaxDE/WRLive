...
export default async function PatchPage({ params }: { params: Promise<{ patch: string }> }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-slate-300">Änderungen (neu/rework/buff/nerf) werden aus der Diff-Pipeline erzeugt.</p>
      <div className="grid gap-2 md:grid-cols-2">{entries.map((e) => <div key={e.slug} className="card">{e.name}: {(e.tags ?? []).join(", ") || "Keine Änderungen"}</div>)}</div>
    </div>
  );
}
